...
class ProgramParser(QObject):

    sgn_file_loaded = pyqtSignal(str, str)  # Full file name, file contents
    ...
    def parse(self, file_name: str, extension: str) -> Program:
        ...
        with open(file) as f:
            contents = f.read()
            ast_root: ast.Module = ast.parse(contents)

        self.program = Program(name=file_name)

        # Recursively parse abstract syntax tree
        self.program.instructions = self.parse_block(ast_root.body)

        self.sgn_file_loaded.emit(file_name + extension, contents)
